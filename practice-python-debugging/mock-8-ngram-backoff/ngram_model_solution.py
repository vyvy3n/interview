...
class NGramModel:
    ...
    def train(self, corpus):
        ...
        self._counts = {}
        self._vocab = None     # invalidate the cached vocab
    ...
    def _backoff_prob(self, context, token, weight):
        ...
        if len(context) == 0:
            unigrams = self._counts.get((), {})
            total = sum(unigrams.values())
            return weight * unigrams.get(token, 0) / total if total else 0.0
    ...
    def logprob(self, context, token):
        ...
        return float(np.log(p)) if p > 0 else -100.0
    ...
    def prune_rare(self, min_count):
        ...
        for context, ctx_counts in self._counts.items():
            for token, count in list(ctx_counts.items()):
                if count < min_count:
                    del ctx_counts[token]

    def token_index(self):
        """Map each DISTINCT token to a contiguous id 0..V-1 (sorted order)."""
        return {tok: i for i, tok in enumerate(sorted(self.vocab))}
    ...
    def smoothed_probs(self, context):
        ...
        return np.maximum(probs, 1e-6)

    def increase_order(self):
        """Bump the n-gram order by 1 (returns nothing; updates self.config)."""
        self.config = self.config._replace(n=self.config.n + 1)
